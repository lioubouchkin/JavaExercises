...
/**
 * This program demonstrates stack and queue operations. The program shows a
 * grid of squares. When the user clicks on one of the squares, a computation is
 * begun that visits all the squares of the grid. As the squares are
 * "encountered", they are colored red. Red squares have been encountered but
 * not yet processed. A square is processed by adding its horizontal and
 * vertical neighbors to the set of encountered squares, if they have not
 * previously been encountered. Once a square has been processed in this way, it
 * is "finished", and it is colored gray. At the end of the process, all the
 * squares are gray. The question is, how does the program decide which red
 * square to process? There can be many red squares waiting for processing. The
 * user can specify one of three methods for deciding which square to process:
 * with a stack, with a queue, or at random. If the random method is chosen,
 * then a red square is chosen for processing at random from among all the red
 * squares. If a queue is used, the red squares are stored on a queue and are
 * processed in FIFO order. If a stack is used, then the squares are processed
 * in LIFO order. (Note: If the user clicks on a white square while a
 * computation is already running, then that square will be "encountered" and
 * added to the set of red squares.) This class contains a main() routine that
 * allows it to be run as a standalone application.
 */
public class DepthBreadth extends JPanel implements MouseListener, ActionListener {

	/**
	 * main() routine just opens a window with a DepthBreadth panel as its
	 * content pane.
	 */
	...
	/**
	 * Represents one square in the grid, by specifying the row number and
	 * column number where it is found.
	 */
	...
	/**
	 * Represents a node in a linked list of Locations. Both the Stack and the
	 * Queue class use this type of linked list.
	 */
	...
	/**
	 * A stack of Locations, with the standard operations, plus a getSize()
	 * method that returns the number of Locations on the stack.
	 */
	...
	/**
	 * A queue of Locations, with the standard operations, plus a getSize()
	 * method that returns the number of Locations on the queue.
	 */
	...
	private final static int SQUARE_SIZE = 12; // Size of one square in the
												// grid.

	private int rows; // Number of rows in the grid. This depends on the size of
						// the panel.
	private int columns; // Number of columns in the grid. This depends on the
							// size of the panel.

	private boolean[][] encountered; // encountered[r][c] is set to true when a
										// square is
										// first encountered. (See comment at
										// top of file.)
										// A square that has been encountered
										// but not
										// finished is red.

	private boolean[][] finished; // finished[r][c] is set to true when a square
									// is
									// finished (i.e. processed). Finished
									// squares are gray.

	private JButton abortButton; // User can click this to terminate the
									// computation.
	...
	private Timer timer; // A timer that drives the computation.
							// When no computation is in progress, timer is
							// null.
	...
	private ArrayList<Location> randomList; // Which one is used depends on the
											// method.
	 ...
	 * @param height
	 *            the height of the panel. The height and width must be passed
	 *            as parameters because they are needed to determine how many
	 *            rows and columns are drawn. The height and width also become
	...
	public DepthBreadth(int width, int height) {
		...
		/*
		 * Determine the number of rows and columns and create the encountered
		 * and finished arrays.
		 */
		...
		JLabel lb = new JLabel("Use:", JLabel.RIGHT); // An unchanging
														// informational label.
	...
	/**
	 * The user has clicked the mouse on the panel. If the user has clicked on a
	 * position in the grid, start a computation to start processing from that
	...
	/**
	 * When the user clicks the button, call doAbort(). Otherwise, this is a
	 * Timer event, so do the next step in the computation.
	 */
	...
	/**
	 * Begin a new computation. Set all the squares back to unencountered and
	 * start a timer that will process the squares beginning with the square at
	 * (startRow,startCol).
	 */
	...
	/**
	 * Do one step in a computation, by processing one location from the stack,
	 * queue, or arraylist. If no more items are available, finish the
	 * computation.
	 */
	...
	/**
	 * Stop the computation, if one is running. This is called when the user
	 ...
	 * encounter it and add it to the data structure. The data structure that is
	 * used depends on the method. Also, display the size of the data structure.
	 */
	...
	/**
	 * Paint the grid of squares. (Other components contained in the panel paint
	 * themselves.)
	 */
	public void paintComponent(Graphics g) {
		...
		/*
		 * Fill the area occupied by the grid with white, then draw black lines
		 * around this area and between the squares of the grid.
		 */
